...
def main(
    ...
    playlist: Annotated[str, typer.Option(help="The name of the playlist")],
    description: Annotated[
        str, typer.Option(help="The description of your playlist")
    ] = "",
    cover: Annotated[
        str, typer.Option(help="Path to your playlist cover (JPG File)")
    ] = None,
):
    artists = []
    tracks = []
    ...
    for search_artist in track(search_artists, description="Getting Artist IDs"):
        artists.append(get_artist_id(artist_name=search_artist.replace("\n", "")))
    ...
    playlist_exists = [item for item in all_user_playlists if item["name"] == playlist]

    if not playlist_exists:
        print("Playlist not existing. Will create it now...")
        work_playlist = spotify_client.user_playlist_create(
            user=current_user["id"], name=playlist, public=True, description=description
        )
    else:
        typer.confirm(
        ...
        work_playlist = spotify_client.playlist(playlist_id=playlist_exists[0]["id"])

        existing_tracks = get_all_tracks_playlist(playlist_id=work_playlist["id"])
        ...
        chunk_size = 100
        chunks = [
            existing_tracks[i : i + chunk_size]
    ...
    for chunk in track(chunks, description="Adding Songs"):
        spotify_client.playlist_add_items(playlist_id=work_playlist["id"], items=chunk)
...
def get_all_tracks_playlist(playlist_id: str) -> list:
    ...
    total_tracks = spotify_client.playlist_items(playlist_id=playlist_id)["total"]
    ...
    for i in range(num_batches):
        ...
        track_batch_uris = [track["track"]["id"] for track in track_batch["items"]]
